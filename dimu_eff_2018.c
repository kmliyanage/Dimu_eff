...
void dimu_eff_2018(){
    ...
    TFile *mc = TFile::Open("DY_effi_2018.root");
    ...
    for(int n=0 ; n<eta ; n++){
        ...
        for(int j=0 ; j<effi ; j++){
            ...
            if(j==3){
                ...
                //----------------------old functions------------------------//
                //TF1 *inclusive_low = new TF1("inclusive_low","pol9",120,660);
                //TF1 *inclusive_high = new TF1("inclusive_high","pol5",660,6000);
                //TF1 *bb_low = new TF1("bb_low","pol2",120,220);
                //TF1 *bb_high = new TF1("bb_high","pol3",220,6000);
                //TF1 *be_low = new TF1("be_low","pol4",120,440);
                //TF1 *be_high = new TF1("be_high","pol5",440,6000);
                
                //----------------------new functions------------------------//
                TF1 *inclusive_low = new TF1("inclusive_low","[0] - [1]*1.0/TMath::Exp((x-[2])/[3]) + [4]*TMath::Power(x,[5])",120,800);
                inclusive_low->SetParameters(1.912,0.5104,84.67,40.95,-3.539,-0.1672);
                TF1 *inclusive_high = new TF1("inclusive_high","[0] + [1]/(TMath::Power((x+[2]),3)) - x*x/[3]",800,6000);
                inclusive_high->SetParameters(0.881,-1.119e+09,1247,8.228e+08);
                TF1 *bb_low = new TF1("bb_low","[0] - [1]*1.0/TMath::Exp((x-[2])/[3]) + [4]*TMath::Power(x,[5])",120,600);
                bb_low->SetParameters(2.14,0.1286,110.6,22.44,-2.366,-0.03382);
                TF1 *bb_high = new TF1("bb_high","[0] + [1]/(x+[2]) - [3]*x",600,6000);
                //bb_high->SetParameters(5.18,-5.845e+04,1.157e+04,0.0002255);
                TF1 *be_low = new TF1("be_low","[0] - [1]*1.0/TMath::Exp((x-[2])/[3]) + [4]*TMath::Power(x,[5])",120,450);
                be_low->SetParameters(13.4,6.693,-4.852e+06,-7.437e+06,-81.43,-1.068);
                TF1 *be_high = new TF1("be_high","[0] + [1]*TMath::Power(x,[2])*1.0/TMath::Exp((x-[3])/[4])",450,6000);
                be_high->SetParameters(0.3154,0.04561,1.362,-4927,727.5);
       ...
       // st->SetY2NDC(0.5);
        ...
        c->SaveAs(etas[n]+"2018.pdf","pdf");
        c->SaveAs(etas[n]+"2018.root","root");
    }
